...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Inconsolata:wght@200..900&family=Nunito:ital,wght@0,688;1,688&display=swap"
        rel="stylesheet">
...
.tablenangcategory{
  ...
  #dispose > .btn-primary:hover {
    background-color: #0056b3; /* Darker on hover */
    transform: translateY(-2px); /* Lift effect */
  }

  #dispose > .btn-secondary {
...
<header class="header">
            <div class="header__container">
                ...
                <p href="#" class="header__logo" style="margin-top: 12px">TRANSACTION</p>
            ...
            <nav class="nav__container">
                <div>
                    <a href="#" class="nav__link nav__logo">
                        ...
                        <span class="nav__logo-name" style="font-size: 15px">TRANSACTION</span>
                    </a>
                            ...
                            <div class="nav__dropdown">
                                ...
                                <div class="nav__dropdown-collapse">
                                    <div class="nav__dropdown-content">
                                        ...
                                        <a href="#" class="nav__dropdown-item">Transaction</a>
...
<button id="resetButton" class="btn mb-3 topbutton"style="margin-right: 10px" ><i class="fa-solid fa-rotate"></i></button>
<button id="downloadBtn" class="btn mb-3 topbutton" onclick="downloadTable()" style="margin-right: 10px"><i class="fa-solid fa-download"></i></button>
    <button id="printBtn" class="btn mb-3 topbutton" onclick="printTable()" style="margin-right: 10px"><i class="fa-solid fa-print"></i></button>
    ...
    <table class="table text-center" id="userTable" style="border-radius: 15px; border-collapse: collapse; width: 100%; margin: 0;">
        <thead class="table sticky-header" style="border-top-left-radius: 15px; border-top-right-radius: 15px;">
            <tr>  
                <th>ORDER NUMBER</th>
                <th>ORDER DATE</th>
                <th>TOTAL AMOUNT</th> <!-- Corrected the spelling -->
                <th>PAYMENT</th>
                <th>Action</th>
            </tr>
        </thead>
...
if ($result) {
    foreach ($result as $row):
        ?>
        <!-- Modal -->
        <div class="modal fade" id="dispose<?= htmlspecialchars($row['order_id']) ?>" tabindex="-1" aria-labelledby="disposeModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-body" style="padding: 20px 10px;">

                        <div class="d-flex justify-content-between align-items-center">
                            <h4 class="mb-10">Item Purchase</h4>
                        ...
                        <div id="tablenaten" class="table-responsive" style="height: 380px; width: 100%; overflow: auto; box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px; border-radius: 15px;">
                            <table class="table text-center" id="userTable" style="border-radius: 15px; border-collapse: collapse; width: 100%; margin: 0;">
                                <thead class="table sticky-header" style="border-top-left-radius: 15px; border-top-right-radius: 15px;">
                                    <tr>
                                        <th>Flavor</th>
                                ...
                                <tbody style="border-bottom-left-radius: 15px; border-bottom-right-radius: 15px;">
                                    <?php
                                    // Query to get details for this specific order
                                    $order_id = mysqli_real_escape_string($conn, $row['OID']); // Use OID for filtering
                                    $detail_sql = "SELECT order_details.id AS order_details_ID,
                                                    flavor.flavor,
                                                    order_details.PRICE,
                                                    order_details.QTY,  
                                                    order_details.TOTAL
                                                    FROM order_details
                                                    INNER JOIN flavor ON order_details.PID = flavor.id
                                                    WHERE order_details.OID = '$order_id'";
                                    ...
                                    if ($detail_result && mysqli_num_rows($detail_result) > 0) {
                                        foreach ($detail_result as $detail_row):
                                            ?>
                                            <tr>
                                                <td><?= htmlspecialchars($detail_row['flavor']) ?></td>
                                                <td><?= htmlspecialchars($detail_row['PRICE']) ?></td>
                                                <td><?= htmlspecialchars($detail_row['QTY']) ?></td>
                                                <td><?= htmlspecialchars($detail_row['TOTAL']) ?></td>
                                            </tr>
                                            <?php
                                        endforeach;
                                    } else {
