...
<header class="header">
            <div class="header__container">
                ...
                <p href="#" class="header__logo" style="margin-top: 12px">Dispose items</p>
 ...
 <div class="nav" id="navbar">
            <nav class="nav__container">
                <div>
                    <a href="#" class="nav__link nav__logo">
                        ...
                        <span class="nav__logo-name" style="font-size: 15px">Dispose_Items</span>
                    </a>
                            ...
                            <div class="nav__dropdown">
                                ...
                                <div class="nav__dropdown-collapse">
                                    <div class="nav__dropdown-content">
                                        ...
                                        <a href="#" class="nav__dropdown-item">Dispose</a>
                                    </div>
                                </div>
        ...
        <div class="main">
         ...
         <div class="d-flex">
        <div class="mb-3">
    <input type="text" id="searchInput" onkeyup="filterTableSS()" placeholder="Search for flavors..."  style="margin-right: 10px" class="form-control" />
</div>
...
$(document).ready(function() {
    ...
    function loadData(query = '', date = '', page = 1) {
        ...
        $.ajax({
            url: 'dispose_live.php',
            type: 'POST',
            dataType: 'json',
            data: { query: query, date: date, page: page },
            success: function(response) {
                $('#tableBody').html(response.content);
                $('#totalDisplay').html('<i class="fa-solid fa-peso-sign"></i> ' + response.totalValue); // Use totalValue here
    ...
    $('#searchInput').on('input', function() { // Corrected ID to #searchInput
    ...
    $('#resetButton').on('click', function() {
        $('#searchInput').val(''); // Corrected ID to #searchInput
        $('#dateInput').val('');
        currentQuery = '';
        currentDate = '';
        currentPage = 1;
...
<!-- <script>
    function filterTableSS() {
        ...
        for (let i = 0; i < tr.length; i++) {
            ...
            if (tds.length >= 5) { // Ensure there are at least 5 columns
                const txtValueCategory = tds[0].textContent || tds[0].innerText;
                const txtValueFlavor = tds[1].textContent || tds[1].innerText;
                const txtValueCol2 = tds[2].textContent || tds[2].innerText; // Column 2
                const txtValueCol3 = tds[3].textContent || tds[3].innerText; // Column 3
                const txtValueCol4 = tds[4].textContent || tds[4].innerText; // Column 4
                
                if (txtValueCategory.toLowerCase().indexOf(filter) > -1 || 
                    txtValueFlavor.toLowerCase().indexOf(filter) > -1 ||
                    txtValueCol2.toLowerCase().indexOf(filter) > -1 ||
                    txtValueCol3.toLowerCase().indexOf(filter) > -1 ||
